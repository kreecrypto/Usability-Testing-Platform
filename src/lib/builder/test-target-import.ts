...
export type TestTargetPreflight = Readonly<{
  access: CapabilityState;
  embed: CapabilityState;
  instrumentation: CapabilityState;
  screen: CapabilityState;
  path: CapabilityState;
  pointer: CapabilityState;
  scroll: CapabilityState;
  coordinates: CapabilityState;
  publishBlocked: boolean;
  reasons: readonly string[];
}>;

export type TestTargetSnapshotV1 = Readonly<{
  provider: TestTargetProvider;
  sourceUrl: string;
  environment: WebEnvironment;
  launchMode: "embed" | "new_tab" | "same_tab" | "unsupported";
  capabilities: TestTargetPreflight;
...
export function preflightTestTarget(input: {
  url: string;
...
export function canPublishTarget(snapshot: TestTargetSnapshotV1): boolean {
  return !snapshot.capabilities.publishBlocked && snapshot.launchMode !== "unsupported";
}
