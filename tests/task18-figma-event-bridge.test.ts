...
function figmaMessage(type: string, data: unknown, source: unknown = expectedSource) {
  return { origin: "https://www.figma.com", source, data: { type, data } };
}
...
const collect = (target: RawTrackingEvent[]) => (event: RawTrackingEvent): void => {
  target.push(event);
};

test("accepts only the exact Figma origin and expected iframe window", async () => {
  const emitted: RawTrackingEvent[] = [];
  const bridge = createFigmaInteractionEventBridge({ expectedSource, session, emitTrackingEvent: collect(emitted), now: () => new Date("2026-09-09T06:20:00.000Z"), eventIdFactory: (sequence) => `00000000-0000-4000-8000-${String(sequence).padStart(12, "0")}` });
  assert.deepEqual(await bridge.handleMessage({ ...figmaMessage("MOUSE_PRESS_OR_RELEASE", mousePayload), origin: "https://evil.example" }), { status: "ignored_origin" });
  assert.deepEqual(await bridge.handleMessage(figmaMessage("MOUSE_PRESS_OR_RELEASE", mousePayload, { iframe: "other" })), { status: "ignored_source" });
...
test("wraps supported Figma pointer evidence in the canonical internal envelope", async () => {
  const emitted: RawTrackingEvent[] = [];
  const bridge = createFigmaInteractionEventBridge({ expectedSource, session, initialScreenId: "9:1", emitTrackingEvent: collect(emitted), now: () => new Date("2026-09-09T06:20:00.000Z"), eventIdFactory: (sequence) => `00000000-0000-4000-8000-${String(sequence).padStart(12, "0")}` });
...
test("keeps navigation sequence/current screen deterministic for later component-state events", async () => {
  const emitted: RawTrackingEvent[] = [];
  const bridge = createFigmaInteractionEventBridge({ expectedSource, session, initialScreenId: "10:1", emitTrackingEvent: collect(emitted), now: () => new Date("2026-09-09T06:21:00.000Z"), eventIdFactory: (sequence) => `10000000-0000-4000-8000-${String(sequence).padStart(12, "0")}` });
  await bridge.handleMessage(figmaMessage("PRESENTED_NODE_CHANGED", { presentedNodeId: "20:1", isStoredInHistory: true, stateMappings: {} }));
  await bridge.handleMessage(figmaMessage("NEW_STATE", { nodeId: "20:component", currentVariantId: "20:a", newVariantId: "20:b", isStoredInHistory: false, isTimedChange: false }));
...
test("keeps access/lifecycle messages operational and outside the usability raw stream", async () => {
  ...
  const bridge = createFigmaInteractionEventBridge({ expectedSource, session, emitTrackingEvent: collect(emitted), onOperationalSignal: (signal): void => { operational.push(signal); }, eventIdFactory: () => "20000000-0000-4000-8000-000000000001" });
  assert.deepEqual(await bridge.handleMessage(figmaMessage("LOGIN_SCREEN_SHOWN", {})), { status: "operational", signal: "login_screen_shown" });
  assert.deepEqual(await bridge.handleMessage(figmaMessage("PASSWORD_SCREEN_SHOWN", {})), { status: "operational", signal: "password_screen_shown" });
  assert.equal(emitted.length, 0);
...
test("fails closed for unsupported or malformed provider messages without consuming sequence", async () => {
  const emitted: RawTrackingEvent[] = [];
  const bridge = createFigmaInteractionEventBridge({ expectedSource, session, emitTrackingEvent: collect(emitted), now: () => new Date("2026-09-09T06:22:00.000Z"), eventIdFactory: (sequence) => `30000000-0000-4000-8000-${String(sequence).padStart(12, "0")}` });
  assert.deepEqual(await bridge.handleMessage(figmaMessage("SCROLL", { y: 100 })), { status: "ignored_unsupported", providerEventType: "SCROLL" });
  assert.deepEqual(await bridge.handleMessage(figmaMessage("MOUSE_PRESS_OR_RELEASE", { presentedNodeId: "10:1" })), { status: "invalid_provider_event", providerEventType: "MOUSE_PRESS_OR_RELEASE" });
  assert.equal(emitted.length, 0);
  assert.equal(bridge.getState().sequence, 0);
...
test("does not advance sequence or screen state when the downstream event sink fails", async () => {
  let fail = true;
  const emitted: RawTrackingEvent[] = [];
  const bridge = createFigmaInteractionEventBridge({ expectedSource, session, initialScreenId: "10:1", emitTrackingEvent: async (event) => { if (fail) throw new Error("sink unavailable"); emitted.push(event); }, now: () => new Date("2026-09-09T06:23:00.000Z"), eventIdFactory: (sequence) => `40000000-0000-4000-8000-${String(sequence).padStart(12, "0")}` });
  await assert.rejects(() => bridge.handleMessage(figmaMessage("PRESENTED_NODE_CHANGED", { presentedNodeId: "20:1", isStoredInHistory: true, stateMappings: {} })), /sink unavailable/);
