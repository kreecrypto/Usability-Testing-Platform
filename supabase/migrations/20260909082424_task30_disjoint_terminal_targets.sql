...
begin
  ...
  if exists (
    select 1
    from unnest(p_success_node_ids) as success(node_id)
    join unnest(p_failure_node_ids) as failure(node_id)
      using (node_id)
  ) then
    raise invalid_parameter_value using message = 'conflicting_terminal_target';
  end if;
